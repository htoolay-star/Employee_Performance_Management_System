...
namespace EPMS.Domain.Services.Performance
{
    public class AppraisalCycleService : IAppraisalCycleService
    {
        ...
        {
            var existing = await _uow.Perf.AppraisalCycles.GetByYearAndTypeAsync(dto.Year, dto.AppraisalType);
            if (existing != null)
            {
                return SuccessResponse<long>.Fail(
                    string.Format(AppraisalCycleMsg.DuplicateCycle, dto.Year, dto.AppraisalType),
                    ErrorType.Conflict);
            }
        ...
        public async Task<SuccessResponse> UpdateAsync(long id, UpdateAppraisalCycleDto dto)
        {
            ...
            cycle.UpdatePeriod(dto.StartDate, dto.EndDate);
